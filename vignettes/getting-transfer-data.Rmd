---
title: "Querying the TRC-10 and TRC-20 transfers"
...
The two functions have very similar arguments. In particular, users can specify the time range of interest with the `min_timestamp` and `max_timestamp` arguments. If no other parameters are provided, both functions will return all transfers of the respective tokens that took place within that time range:
...
To retrieve transfers of a specific TRC-10 token, one can supply the `owner_address` argument to the `get_trc10_transfers()`. As the name suggests, this argument expects the address of the token's creator. If unknown, this address can be found using the function `get_trc10_token_description()`.
...
In case with the TRC-20 tokens, one instead would need to supply `get_trc20_transfers()` with the `contract_address` argument, that is address of the token's smart contract:
...
Results returned in the last two examples could be made even more specific by providing the `related_address` argument. This would allow one to retrieve transfers to or from that related address:
