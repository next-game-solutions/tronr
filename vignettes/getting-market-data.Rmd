---
title: "Getting the market data for Tronix (TRX)"
...
[Tronix](https://tron.network/trx?lng=en) (TRX, a.k.a. Tron) is the native currency of the TRON network. The TRX token is based on the [ERC-20 Etherium Standard](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/) and is fully compatible with it. Although the original purpose of TRX was to enable payments for digital entertainment powered by the TRON network, nowadays TRX has gained many other use cases that power transactions on and generate value of the chain.

One can purchase TRX or swap it for other cryptocurrencies on many exchanges (e.g., Binance, Huobi, Bittrex, etc.). In January 2021, TRX's global rank among cryptocurrencies was 17, with a total market cap reaching over $3B.

## TRX market data that can be obtained using `tronr`

The `tronr` package contains several functions that one can use to query TRX market data offered by the public [CoinGecko API](https://www.coingecko.com/api/documentations/v3). At a high level, the following data can be retrieved:
...
* TRX market metrics for a range of most recent historical dates (e.g., last 7 days).

All of these functions return [tibbles](https://tibble.tidyverse.org/) that are directly suitable for any downstream analysis tasks. Provided below are a few examples of querying the TRX market data.

Please note that the CoinGecko API has a limit of 100 calls per minute. Keep this in mind when developing your applications using `tronr`.
...
In addition, one can use the `get_current_trx_price()` function to retrieve data on the current market cap, trading volume over the last 24 hours, and price percentage change compared to 24 hours ago. This can be done by turning on the `include_market_cap`, `include_24h_vol`, and `include_24h_change` options:
...
TRX market data for a specific historical date can be obtained with the
`get_trx_market_data_for_date()` function. The `date` argumet of this function accepts character, Date, or POSIXct values in the `%Y-%m-%d` format, e.g.:
...
The history of TRX market data offered by CoinGecko starts on `2017-11-09 00:00:00`. Attempts to retrieve data for earlier dates will fail with the respective console message:
...
One can also retrieve TRX market data for a range of historical dates. This can be done using the `get_trx_market_data_for_date_range()` function. In contrast to `get_trx_market_data_for_date()`, this function only accepts a single base currency at a time (argument `vs_currency`). In addition, timestamps that defining the start (`from_date`) and end (`to_date`) of historical range are to be POSIXct values in the `%Y-%m-%d %H:%M:%S` format.

Granularity of data returned by `get_trx_market_data_for_date_range()` depends on the requested time range. Hourly data will be retrieved for periods of up to     90 days, and daily data for periods above 90 days:

```{r date-range}
# Range of less than 1 day:
# get_trx_market_data_for_date_range(
#   vs_currency = "usd",
#   from_date = as.POSIXct("2020-01-01 00:00:00", tz = "UTC"),
#   to_date = as.POSIXct("2020-01-01 05:00:00", tz = "UTC")
# )
...
#   vs_currency = "usd",
#   from_date = as.POSIXct("2020-01-01 00:00:00", tz = "UTC"),
#   to_date = as.POSIXct("2020-05-01 00:00:00", tz = "UTC")
# )
```

Similar to `get_trx_market_data_for_date()`, `get_trx_market_data_for_date_range()` will fail if `from_date` and/or `to_date` define a time range for which no data exist:

```{r date-range-errors, error=TRUE}
# Requesting non-existing data from the past:
# get_trx_market_data_for_date_range(
#   vs_currency = "usd",
#   from_date = as.POSIXct("2017-11-08 00:00:00", tz = "UTC"),
#   to_date = as.POSIXct("2020-01-01 00:00:00", tz = "UTC")
# )
...
# get_trx_market_data_for_date_range(
#   vs_currency = "usd",
#   from_date = as.POSIXct("2021-01-01 00:00:00", tz = "UTC"),
#   to_date = as.POSIXct("2090-01-01 00:00:00", tz = "UTC")
# )
```

## Querying the last _n_ days

As a variant of querying a range of historical dates, one can use the `get_trx_market_data_for_last_n_days()` function to retrieve TRX market data for several last days (as of the current time):
...
# Less than 90 days, with with `interval = "daily"`:
# get_trx_market_data_for_last_n_days(
#   vs_currency = "usd",
#   days = 10,
#   interval = "daily"
...
# More than 90 days, with with `interval = "daily"`:
# get_trx_market_data_for_last_n_days(
#   vs_currency = "usd",
#   days = 100,
#   interval = "daily"
