...
test_that("get_trc20_balance throws errors as expected", {

  expect_error(get_trc20_balance(address = 1L,
                                 only_confirmed = FALSE,
                                 max_attempts = 3L))

  expect_error(get_trc20_balance(address = correct_address,
                                 only_confirmed = 1L,
                                 max_attempts = 3L))
  ...
  expect_error(suppressMessages(get_trc20_balance(address = wrong_address,
                                                  only_confirmed = FALSE,
                                                  max_attempts = 1L)))
})


test_that("get_trc20_balance returns correct objects", {

  result <- get_trc20_balance(address = correct_address,
                              only_confirmed = TRUE,
                              max_attempts = 3L)

  result2 <- get_trc20_balance(address = correct_address2,
                               only_confirmed = TRUE,
                               max_attempts = 3L)
