...
function_params <- function(arguments) {
  ...
  descriptions <- c(
    ...
    address = "@param address (character) - address of the account of interest,
               either in `base58check` (starts with `T`) or `hex`
               (starts with `41`) format.",
    ...
    asset_id = "@param asset_id (character) - ID of the TRC-10 token of interest,
               presented either as a set of numbers (`1002762`) or as address of
               the owner who issued that token (in `base58hex` or `hex` format).
               Using either of this IDs is possible because there is a 1:1
               relationship between them, i.e. an account is only allowed to
               issue a single TRC-10 token.",
